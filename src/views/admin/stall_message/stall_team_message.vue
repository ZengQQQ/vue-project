<template>
    ...
    <div class="table">
      <el-table :data="tableData" stripe>
        <el-table-column prop="stm_idst_idt_id" label="社团活动ID" width="150"></el-table-column>
    <el-table-column prop="stm_info" label="社团活动信息"></el-table-column>
    ...
    <el-table-column prop="stm_pass" label="社团活动申请状态">
      <template slot-scope="scope">
        <el-tag type="warning" v-if="scope.row.stm_pass === 0">考虑中</el-tag>
        <el-tag type="success" v-else-if="scope.row.stm_pass === 1">同意</el-tag>
        <el-tag type="danger" v-else>拒绝</el-tag>
      </template>
    </el-table-column>
    <el-table-column prop="stm_status" label="社团活动状态">
      <template slot-scope="scope">
        <el-tag v-if="scope.row.stm_status === 0">正常</el-tag>
        <el-tag type="danger" v-else-if="scope.row.stm_status === 1">删除</el-tag>
        <el-tag type="danger" v-else-if="scope.row.stm_status === 2">封禁</el-tag>
        <el-tag type="warning" v-else-if="scope.row.stm_status === 3">审核中</el-tag>
    ...
    <el-table-column prop="stm_dct" label="社团活动申请类型">
      <template slot-scope="scope">
        <el-tag v-if="scope.row.stm_dct === 0">邀请</el-tag>
        <el-tag type="success" v-else>申请</el-tag>
      </template>
    </el-table-column>
    <el-table-column prop="stm_time" label="社团活动申请时间"></el-table-column>
    ...
    <div class="edit">
      <el-dialog title="编辑" v-model="showEditDialog">
        <el-form :model="currentData" label-width="100px">
            <el-form-item label="社团活动ID">
        <el-input v-model="formData.stm_idst_idt_id" disabled></el-input>
      </el-form-item>
      <el-form-item label="社团活动信息">
        <el-input type="textarea" :rows="3" v-model="formData.stm_info"></el-input>
      </el-form-item>
      ...
      <el-form-item label="社团活动申请状态">
        <el-select v-model="formData.stm_pass">
          <el-option value="0">考虑中</el-option>
          <el-option value="1">同意</el-option>
          <el-option value="2">拒绝</el-option>
        </el-select>
      </el-form-item>
      <el-form-item label="社团活动状态">
        <el-select v-model="formData.stm_status">
          <el-option value="0">正常</el-option>
          <el-option value="1">删除</el-option>
      ...
      <el-form-item label="社团活动申请类型">
        <el-select v-model="formData.stm_dct">
          <el-option value="0">邀请</el-option>
          <el-option value="1">申请</el-option>
        </el-select>
      </el-form-item>
      <el-form-item label="社团活动申请时间">
        <el-date-picker v-model="formData.stm_time" type="datetime" format="yyyy-MM-dd HH:mm:ss"></el-date-picker>
      </el-form-item>
        </el-form>
