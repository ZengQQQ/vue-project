        ...
        <el-header>
          <el-form :model="queryReceived">
            <el-row :gutter="20">
              ...
              <el-col :span="6">
                <el-button type="primary" @click="searchB(queryReceived)">搜索</el-button>
...
function resetQueryReceived() {
  ...
  fetchReceive(queryReceived.value, 1);
}
...
function resetQuerySend() {
  ...
  fetchReceive(querySend.value, 1);
}


// 搜索接受
function searchB(data) {
  fetchReceive(data.value, 1);
}
...
// ???
// 初始化数据
fetchSend(queryReceived.value, 1);


// 请求邀请数据
async function fetchReceive(data, c_page) {
