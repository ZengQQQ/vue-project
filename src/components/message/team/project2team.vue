    ...
    <el-tabs v-model="activeTab">
      ...
      <el-main>
        <el-table :data="tableData">
          <el-table-column prop="stallFix.teamFix.t_name" label="队伍" width="150"></el-table-column>
          ...
          <el-table-column label="操作" width="230">
            <template #default="{ row }">
              ...
              <!-- <hr> -->
              <!-- <el-button size="default" @click="handleApply(row, 1)">同意</el-button>
  ...
  try {
    ...
    if (res.code === 200) {
      if (res.data.totalSize === 0) {
        ...
        // 筛选spm_dct: 0, 邀请数据
        tableData.value = res.data.listPage.filter(item => item.spm_dct === 0);
...
const handleProjectDetail = (row) => {
  console.log("row", row.stallFix.project);
  detailData.value = row.stallFix.project;
