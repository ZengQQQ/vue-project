        ...
        <el-main>
          <el-table :data="tableData">
            ...
            <el-table-column label="操作" width="230">
              <template #default="{ row }">
                <el-button size="default" @click="handleDetail(row)">查看队伍</el-button><br>
                <hr>
  ...
  try {
    ...
    if (res.code === 200) {
      if (res.data.totalSize === 0) {
        ...
        // 筛选spm_dct: 1, 申请数据
        tableData.value = res.data.listPage.filter(item => item.spm_dct === 1);
...
const handleDetail = (row) => {
  ...
  detailData.value = row.stallFix.teamFix;
